...
<?php
	...
	$options = getopt('hi:k:l:o:rt:');
	if (isset($options['h']))
		exit(file_get_contents('help.txt'));
	if (isset($options['i']))
		$input_path = $options['i'];
	$language_key = $options['k'] ?? false;
	if (isset($options['l']))
		$targets = $options['l'];
	if (isset($options['o']))
	...
	function process_file(string $input_path, string $output_file_name): void {
		...
		foreach ($targets as $target) {
			...
			array_walk_recursive($output, function(&$value, $key) use ($target) {
				if (is_string($value)) {
					$value = translate_string($value, $target);
				}
			});
			$output_path = "$output_dir/$target/$output_file_name";
			if (!is_dir("$output_dir/$target"))
				mkdir("$output_dir/$target", 0777, true);
			put_results($output_path, $output, $output_type);
		}
		else {
			$output = $input;
			check_node_for_keys($output, $targets);
	...
	function check_node_for_keys(array &$value, array $targets): void {
		global $language_key;
		if (is_array($value))
			if (isset($value[$language_key]))
				foreach ($targets as $target)
					$value[$target] = translate_string($value[$language_key], $target);
			else
				foreach ($value as &$child)
					if (is_array($child))
						check_node_for_keys($child, $targets);
	}

	function translate_string(string $value, string $target): string {
		global $api_keys;
		global $char_counts;
	...
	function put_results(string $output_path, $output, string $output_type): void {
		echo "Putting results in $output_path...\n";
		switch ($output_type) {
			case 'yaml':
